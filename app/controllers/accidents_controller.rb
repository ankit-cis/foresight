class AccidentsController < ApplicationController
  ...
  # GET /accidents.json
  def index
    exception_id = 'b57cb701-1a37-4ddc-89a8-8703b37f9b16'
    include_this_user = current_user.id == exception_id

    @accidents = Accident.company_secure.all.order(created_at: :desc).page params[:page]

    if current_user.is_admin
       @search =  include_this_user ? Accident.ransack(params[:q]) : Accident.where.not(user_id: exception_id).ransack(params[:q])
    else
       @search = include_this_user ? Accident.company_secure.ransack(params[:q]) : Accident.company_secure.where.not(user_id: exception_id).ransack(params[:q])
    end
